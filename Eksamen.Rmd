...
##4.3 Heatmap beregnet med distanse
<<<<<<< HEAD
...
```{r}
data.norm.matrix <- as.matrix(data.norm[,-(1:3)])
data.norm.matrix.log <- log(data.norm.matrix + 1)
data.norm.matrix.log.scaled <- scale(data.norm.matrix.log, center = TRUE, scale = TRUE) 

dist.var <- dist(t(data.norm.matrix.log.scaled)) 
dist.var.tre <- hclust(dist.var)  
plot(dist.var.tre) 

dist.obs <- dist(data.norm.matrix.log.scaled)
...
##Prøvefelt for heatmap
```{r}
#Distanse

data.norm <- data.norm[1:10,1:13]
data.norm.matrix <- as.matrix(data.norm[,-(1:3)]) 
data.norm.matrix.log.scaled <- scale(data.norm.matrix, center = TRUE, scale = TRUE) 
...
heatmap.2(x = data.norm.matrix.log.scaled, 
>>>>>>> 37555382b5b5ed76625532e721b47416b662969b
          Rowv = as.dendrogram(dist.obs.tre),
          Colv = as.dendrogram(dist.var.tre),
          trace = "none",
          col = colorRampPalette(c("blue","white","red")),
